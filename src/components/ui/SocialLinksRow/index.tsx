...
import { Instagram, Linkedin, Twitter, Mail, Github, Globe } from "lucide-react";
...
interface SocialLinksRowProps {
  links: SocialLinks;
  /** Website fallback URL if not in links but exists in parent (e.g. Sponsors) */
  websiteUrl?: string;
  className?: string;
}

export function SocialLinksRow({ links, websiteUrl, className }: SocialLinksRowProps) {
  const tCommon = useTranslations("Common");

  const showWebsite = links.website || websiteUrl;

  return (
    <div className={cn("flex flex-wrap gap-3", className)}>
      {showWebsite && (
        <a 
          href={links.website || websiteUrl} 
          target="_blank" 
